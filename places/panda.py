...
class PandaExpress(Restaurant):
    ...
    reg_hours = {'open': {'hour': 10, 'min': 30},
                 'close': {'hour': 21, 'min': 30}}
    reg_fri_hours = {'open': {'hour': 10, 'min': 30},
                     'close': {'hour': 20, 'min': 0}}
    reg_weekend_hours = {'open': {'hour': 12, 'min': 0},
                         'close': {'hour': 19, 'min': 0}}
    ...
    def get_hours(self, dt):
        ...
        if dt.date() >= summer['start'] and dt.date() <= summer['end']:
            ...
            if dt.weekday() == 4:
                return {'open': time(
                        self.reg_fri_hours['open']['hour'],
                        self.reg_fri_hours['open']['min'], 0),
                        'close': time(
                        self.reg_fri_hours['close']['hour'],
                        self.reg_fri_hours['close']['min'], 0)}
            if dt.weekday() >= 5:
                return {'open': time(
                        self.reg_weekend_hours['open']['hour'],
                        self.reg_weekend_hours['open']['min'], 0),
                        'close': time(
                        self.reg_weekend_hours['close']['hour'],
