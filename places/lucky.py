...
class LuckyKitchen(Restaurant):
    ...
    reg_hours= {'open': {'hour': 11, 'min': 0}, 'close': {'hour': 22, 'min': 0}}
    reg_fri_sat_hours= {'open': {'hour': 11, 'min': 0}, 'close': {'hour': 22, 'min': 30}}
    reg_sun_hours= {'open': {'hour': 12, 'min': 0}, 'close': {'hour': 22, 'min': 30}}
...
#        else:
            if dt.weekday() == 4 or dt.weekday() == 4:
                return {'open': time(self.reg_fri_sat_hours['open']['hour'],
                                self.reg_fri_sat_hours['open']['min'], 0),
                        'close': time(self.reg_fri_sat_hours['close']['hour'],
                                self.reg_fri_sat_hours['close']['min'], 0)}
            if dt.weekday() == 6:
