...
class Quiznos(Restaurant):
    summer_start = {'month': 5, 'day': 1}
    summer_end = {'month': 8, 'day': 31}
    summer_hours = {'open': {'hour': 10, 'min': 0},
                    'close': {'hour': 20, 'min': 0}}
    summer_sat_hours = {'open': {'hour': 11, 'min': 0},
                        'close': {'hour': 19, 'min': 0}}
    summer_sun_hours = {'open': {'hour': 12, 'min': 0},
                        'close': {'hour': 19, 'min': 0}}
    reg_hours = {'open': {'hour': 8, 'min': 0},
                 'close': {'hour': 21, 'min': 0}}
    reg_sat_hours = {'open': {'hour': 10, 'min': 0},
                     'close': {'hour': 20, 'min': 0}}
    ...
    def get_hours(self, dt):
        ...
        if dt.date() >= summer['start'] and dt.date() <= summer['end']:
            if dt.weekday() == 5:
                return {'open': time(
                        self.summer_sat_hours['open']['hour'],
                        self.summer_sat_hours['open']['min'], 0),
                        'close': time(
                        self.summer_sat_hours['close']['hour'],
                        self.summer_sat_hours['close']['min'], 0)}
            if dt.weekday() == 6:
                return {'open': time(
                        self.summer_sun_hours['open']['hour'],
                        self.summer_sun_hours['open']['min'], 0),
                        'close': time(
                        self.summer_sun_hours['close']['hour'],
