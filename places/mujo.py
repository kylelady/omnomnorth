...
class Mujo(Restaurant):
    ...
    spring_hours = {'open': {'hour': 8, 'min': 0},
                    'close': {'hour': 19, 'min': 0}}
    spring_fri_hours = {'open': {'hour': 8, 'min': 0},
                        'close': {'hour': 18, 'min': 0}}
    summer_hours = {'open': {'hour': 8, 'min': 0},
                    'close': {'hour': 18, 'min': 0}}
    ...
    def get_hours(self, dt):
        ...
        if dt.date() >= spring['start'] and dt.date() <= spring['end']:
            ...
            if dt.weekday() == 4:
                    return {'open': time(
                            self.spring_fri_hours['open']['hour'],
                            self.spring_fri_hours['open']['min'], 0),
                            'close': time(
                            self.spring_fri_hours['close']['hour'],
                            self.spring_fri_hours['close']['min'], 0)}
            return {'open': time(
                    self.spring_hours['open']['hour'],
                    self.spring_hours['open']['min'], 0),
                    'close': time(
                    self.spring_hours['close']['hour'],
        ...
        if dt.date() >= summer['start'] and dt.date() <= summer['end']:
            ...
            if dt.weekday() == 4:
                return {'open': time(
                        self.summer_fri_hours['open']['hour'],
                        self.summer_fri_hours['open']['min'], 0),
                        'close': time(
                        self.summer_fri_hours['close']['hour'],
            ...
            if dt.time() < time(4):  # late-night hours
                if 0 <= dt.weekday() - 1 <= 5:
                    return {'open': time(
                            self.reg_night_hours['open']['hour'],
                            self.reg_night_hours['open']['min'], 0),
                            'close': time(
                            self.reg_night_hours['close']['hour'],
                            self.reg_night_hours['close']['min'], 0)}
            if dt.weekday() == 5:
                return {'open': time(
                        self.reg_sat_hours['open']['hour'],
                        self.reg_sat_hours['open']['min'], 0),
                        'close': time(
                        self.reg_sat_hours['close']['hour'],
                        self.reg_sat_hours['close']['min'], 0)}
            if dt.weekday() == 6:
                return {'open': time(
                        self.reg_sun_hours['open']['hour'],
                        self.reg_sun_hours['open']['min'], 0),
                        'close': time(
                        self.reg_sun_hours['close']['hour'],
